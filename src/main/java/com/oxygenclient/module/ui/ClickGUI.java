...
public class ClickGUI extends Screen {
    ...
    protected void init() {
        ...
        for (Module mod : mods) {
            ...
            String display;
            if (binding) {
                display = "§e⌨ Press key... (ESC = remove)";
            } else {
        ...
        if (bindingModule != null) {
            this.addDrawableChild(ButtonWidget.builder(
                Text.literal("§e§l⌨ Binding: §f" + bindingModule.getName()), b -> {}
            ).dimensions(200, y + 5, 200, 20).build());
            this.addDrawableChild(ButtonWidget.builder(
                Text.literal("§7Press key §8| §cESC §7= Remove"), b -> {}
            ).dimensions(200, y + 27, 200, 20).build());
            this.addDrawableChild(ButtonWidget.builder(
                Text.literal("§cCancel"), b -> { bindingModule = null; clearChildren(); init(); }
            ).dimensions(200, y + 49, 80, 20).build());
        }

        this.addDrawableChild(ButtonWidget.builder(
            Text.literal("§c✕"), b -> close()
        ).dimensions(this.width - 25, 2, 20, 20).build());
    }

    private void renderSettings(int y) {
        this.addDrawableChild(ButtonWidget.builder(
            Text.literal("§d§l✧ " + selectedModule.getName()), b -> {}
    ...
    public void render(DrawContext ctx, int mx, int my, float d) {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            ctx.fill(5, 22 + i, width - 5, 23 + i, 
                (200 << 24) | ((int)(255*r + 147*(1-r)) << 16) | 
                ((int)(105*r + 50*(1-r)) << 8) | (int)(180*r + 139*(1-r)));
        }
        ctx.drawTextWithShadow(textRenderer, "§d✧ §fOxyGen §7v" + OxygenClient.VERSION, 12, 6, 0xFFFFFF);
        ctx.drawTextWithShadow(textRenderer, "§7" + selected.getName() + " §8• §7SHIFT:Bind CTRL:Settings ESC:Close/Unbind", 10, 36, 0xAAAAAA);
